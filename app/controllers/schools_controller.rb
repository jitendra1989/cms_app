...
def room
    @room=MgInstituteRoom.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id]).paginate(page: params[:page], per_page: 10)
  end
  ...
  def room_create
    @room=MgInstituteRoom.new(params_room)
    @room.save
    redirect_to :action=>"room"
  end

  def room_edit
    @room=MgInstituteRoom.find(params[:id])
    render :layout => false
  ...
  def room_update
    @room=MgInstituteRoom.find(params[:id])
    @room.update(params_room_update)
    redirect_to :action=>"room"
  end

  def room_delete
    @room=MgInstituteRoom.find(params[:id])
    @room.update(:is_deleted=>1)
    redirect_to :action=>"room"
  end
  ...
  def school_params
    params.require(:school).permit(:subdomain,:logo,:is_deleted, :school_name,:school_code,:address_line1, :address_line2, :street, :city, :state,:pin_code, :landmark,:country,:mobile_number,:email_id,:fax_number,:date_format,:timezone,:currency_type,:affilicated_to,:grading_system,:image_file,:reg_num,:language,:start_time,:end_time,:mg_leave_calendar_start_date,:mg_timetable_type)
  end
   ...
   #params.require(:save).permit(:School_id,:custom_field_id,:one['value'],:two['value'],:three['value'],:four['value'])
    ...
    def params_room
      params.require(:room).permit(:room_no,:mg_employee_specialization_id,:capacity,:is_deleted,:mg_school_id,:created_by,:updated_by,:mg_room_type)
    end

    def params_room_update
      params.require(:room).permit(:room_no,:mg_employee_specialization_id,:capacity,:is_deleted,:mg_school_id,:updated_by,:mg_room_type)
    end
