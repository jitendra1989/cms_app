 ...
 puts "xxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
  ...
  def leave_details_for_employee
    ...
    employee = MgEmployee.find(params[:id])
       ...
       @levee_typs=MgEmployeeLeaveType.where("mg_school_id=#{session[:current_user_school_id]} AND is_deleted  = 0 AND minimum_month_experience <= ? AND gender IN (?) AND mg_employee_type_id =? AND marital_status IN (?)",emp_experience, emp_gender, emp_type_id,emp_marital_status)
     ...
     # date_array=((from_date)..to_date).map{|d|[d.year, d.month] }
       ...
       # puts "joining_date--------------:#{Date.parse(employee_joing_date[0].joining_date.to_s)}"
        ...
        if from_date.to_date<=Date.parse(employee_joing_date[0].joining_date.to_s).to_date
         ...
         puts "step 7.1"
         #puts "from_date : "+from_date.to_s 
         #puts "@last_date : "+@last_date.to_s 
        @fulldayObject=nil
        if(from_date.present?  && @last_date.present?)
