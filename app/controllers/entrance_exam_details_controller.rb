class EntranceExamDetailsController < ApplicationController
  ...
  layout "mindcom"
    ...
    @entrance_exam_details.update(:is_deleted=>1, :updated_by=>session[:user_id])
	...
	def exam_venue
		@exam_venue = MgEntranceExamVenue.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id]).paginate(page:params[:page],per_page:10)
	end
	...
	def destroy_exam_venue	
		...
		@exams_venues[0].update(:is_deleted=>1,:updated_by=>session[:user_id])
		respond_to do |format|
    ...
    if params[:options_for_select_exam] == "selectentranceexam"
      if @shortlisted_for_exam.present?
        @shortlisted_for_exam.each do |stu_id|
          @mg_student = MgStudentAdmission.find_by(:id=>stu_id).update(:is_selected_for_entrance_test=>'accepted', :updated_by=>session[:user_id])
        end
      end
   else
     if @shortlisted_for_exam.present?
       @shortlisted_for_exam.each do |stu_ids|
         @mg_student = MgStudentAdmission.find_by(:id=>stu_ids).update(:is_selected_for_entrance_test=>'rejected', :updated_by=>session[:user_id])
       end
     end
    end
    flash[:notice]="Successfully Updated"
	...
	def update_student_marks
		...
		if @student_ids.present?
      	...
      	if @mg_student.present?
      		@mg_student.update_all(:updated_by=>session[:user_id],:exam_total_marks=>params[:exam_total_marks][i],:obtained_marks=>params[:obtained_marks][i])      	
				end
			end
		end
    ...
    if params[:option]=="1"
      if @shortlisted_for_admission.present?
        @shortlisted_for_admission.each do |stu_id|
          ...
          @mg_student.update(:is_shortlisted_for_admission=>'accepted',:updated_by=>session[:user_id])
         end
      end
   else
     if @shortlisted_for_admission.present?
       @shortlisted_for_admission.each do |stu_ids|
         @mg_student = MgStudentAdmission.find_by(:id=>stu_ids).update(:is_shortlisted_for_admission=>'rejected',:updated_by=>session[:user_id])
       end
      end
    end
    flash[:notice]="Successfully Updated"
  ...
  def update_student_index
    ...
    if params[:index].present?
      params[:index].each do |key, value|
        ...
        @mg_student.update(:selection_index=>value, :updated_by=>session[:user_id])
      end
    end
