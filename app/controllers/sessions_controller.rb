class SessionsController < ApplicationController
 ...
 #com
  ...
  def create
    ...
    #Added By Bindhu
    # user = MgUser.authenticateUsers(params[:user_name], params[:password],deleted[0])  
    if user
     ...
     # Iterating array
      ...
      puts "Step -- 2 "
          ...
          #Bindhu Added for Accounts login starts  
           ...
           elsif @user_type == 'examination_controller'
             redirect_to :controller => '/exam_management/invigilation_details/'
           elsif @user_type == 'hod_login'
             redirect_to :controller => '/exam_management/invigilators/'
          ...
          # Added By Bindhu for doctor login starts
           ...
           elsif  @user_type == "placement_cell_head"
             # puts ahsdjkad
              redirect_to :controller => 'placements/interview_rounds'

               elsif  @user_type == "hostel_admin"
             # puts ahsdjkad
              redirect_to :controller => 'hostel_management/hostel_application_list'
           # Added By Bindhu for doctor login ends
         elsif  @user_type == "recruitment_assistant"
             # puts ahsdjkad
              redirect_to :controller => 'placements/placements_registration'
              elsif  @user_type == "sports_incharge"
             # puts ahsdjkad
              redirect_to :controller => 'sports_management/game'
              
          elsif @user_type=="Company"
              redirect_to :controller => 'placements/corporate_profile'
  ...
  def change_school
   ...
   puts params[:principal][:mg_school_id]
    ...
    # redirect_to :controller=>'dashboards', :action=>'principle', :subdomain=>"#{school.subdomain}.school"

    redirect_to  url_for(:controller => "dashboards", :action => "principal",:subdomain => "#{school.subdomain}.#{request.subdomain.split('.')[1]}")
