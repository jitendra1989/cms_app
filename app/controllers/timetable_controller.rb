 ...
 def delete_time_table
  ...
  def batch_wise_select_class_timings_name
    ...
    if @batch_id.present? 
      ...
      @classTimingNameTable=MgClassTiming.where(:mg_wing_id=>@wing_id,:is_deleted=>0, :mg_school_id=>session[:current_user_school_id],:is_break=>0).order(:mg_weekday_id, :start_time)#.first(2)#, MgWeekday.find(:mg_weekday_id).weekday
       ...
       # @subjects=MgSubject.where(:is_deleted=>0,:id=>@SubjectIds,:mg_school_id=>session[:current_user_school_id])
       # @subjects.each do |s|
       # @rooms=MgInstituteRoom.where(:mg_employee_specialization_id=>s.mg_employee_specialization_id).pluck(:room_no,:id)
       # end
  ...
  def batch_wise_week_day_time_table
    ...
    @employee=MgEmployee.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id]).pluck(:first_name,:id)
    @rooms=MgInstituteRoomm.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id]).pluck(:room_no,:id)
  ...
  def teacher_for_subject
    if params[:sub]=='Subject'
      ...
      # subject_specilization_id=MgSubject.find_by(:id=>params[:subject]).pluck(:mg_employee_specialization_id)
      # @rooms="select a.room_no, a.id from mg_institute_rooms a where a.mg_employee_specialization_id=subject_specilization_id"
      # puts "SubjectId"
      # puts params[:subject]
  ...
  def teacher_for_subject_save 
    ...
    @elective_room_id=params[:elective_room_id]
    ...
    @mandatory_room_id=params[:mandatory_room_id]

    # @mandatorys_rooms_ids=params[:mandatorys_rooms_ids]

    

    puts @mandatory_room_id.inspect
    # logger.inhkjasdhf
  ...
  def class_timings_for_selected_batch
    ...
    elsif params[:key_select]=="show_elective_group"
      entry=MgTimeTableEntry.where(:is_deleted=>0, :mg_batch_id=>params[:mg_batch_id], :mg_school_id=>mg_school_id, :mg_class_timings_id=>params[:mg_class_timings_id], :mg_elective_group_id=>params[:mg_elective_group_id])
      str='<table class="mg-showdata-bg mg-tbl-layout" cellspacing="1" cellpadding="1" border="1">'
      str +='<tr>'
          str +="<th class='mg-td-width-half' valign='top'>Room Number</th>"
          str +="<th class='mg-td-width-half' valign='top'>Subject Name</th>"
          str +="<th class='mg-td-width-half' valign='top'>Employee Name</th>"
        str +='</tr>'
        count=0
      entry.each do |e|
        str +='<tr>'
          str +="<td>#{e.mg_institute_room.try(:room_no)}</td>"
          str +="<td>#{e.mg_subject.try(:subject_name)}</td>"
          str +="<td>#{e.mg_employee.try(:employee_full_name_with_number)}</td>"
        str +='</tr>'
      end
      str +='</table>'
      str += '<p class="mg-p-mgtop"><a class="mg-small-form-btn cancel-form-dialog">Cancel</a></p>'
      @object={str: str}
    ...
    elsif params[:key_select]=='subject_type'
          @object=MgSubject.where(:is_deleted=>0, :mg_school_id=>mg_school_id, :mg_subject_type_id=>params[:mg_subject_type_id])
  ...
  def select_batch_for_show_time_table
   ...
   mg_school_id=session[:current_user_school_id]
    ...
    # puts subject.pluck(:id)
    # puts "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
    # uniq_subject=subject.select(:id).uniq(:mg_elective_group_id)#.map {|a| a.mg_elective_group_id}#.uniq(:mg_elective_group_id)#.uniq { |p| p.mg_elective_group_id }
    # puts uniq_subject.inspect
    ...
    puts "---------------------------------------------"
      ...
      time_table_arr=timetables.student_show_time_table(params[:batch_id], mg_school_id, @academic_years[0].try(:id), session[:user_type], session[:user_id])
...
school_id = session[:current_user_school_id]
 ...
 begin
   ...
   puts params[:select_key]
    if params[:select_key]=="all"
	
      @notice = gtt.create_time_table(school_id)
    elsif params[:select_key]=='spl'
      @notice=gtt.create_time_table_specialization(school_id, params[:mg_employee_specialization_id])
    end
   rescue Exception => ex
...
#   end
 ...
 mg_school_id=session[:current_user_school_id]
  if params[:mg_employee_specialization_id].present?

    # subject=MgSubject.where(:is_deleted=>0, :mg_school_id=>mg_school_id, :mg_employee_specialization_id=>params[:mg_employee_specialization_id]).pluck(:id)
    # entry=MgTimeTableEntry.where(:is_deleted=>0, :mg_school_id=>mg_school_id, :mg_subject_id=>subject.present? ? subject : [])
   specialization =MgParticularSpecialization.where(:mg_employee_specialization_id=>params[:mg_employee_specialization_id], :is_deleted=>0, :mg_school_id=>mg_school_id)



   puts "specialization------------------ 1"
   puts specialization.pluck(:mg_employee_id).inspect
   puts ""
   batch =MgBatch.where(:is_deleted=>0, :mg_school_id=>mg_school_id, :mg_employee_id=>specialization.pluck(:mg_employee_id)).pluck(:id)
 
   entry=MgTimeTableEntry.where(:is_deleted=>0, :mg_school_id=>mg_school_id, :mg_batch_id=>batch)
   entry.update_all(:is_deleted=>1)
  else
    timetabledelete=MgTimeTableEntry.where(:is_deleted=>0, :mg_school_id=>mg_school_id)
    # @timetabledelete.each do |timetable|
    #   timetable.update(:is_deleted => 1)
    # end
    timetabledelete.update_all(:is_deleted=>1)
  end
...
def change_teacher_save
  ...
  school=MgSchool.find(mg_school_id)
  time_table=MgTimeTableEntry.where(:is_deleted=>0, :mg_school_id=>mg_school_id , :mg_batch_id=>params[:mg_batch_id], :mg_class_timings_id=>params[:mg_class_timing_id], :mg_timetable_id=>params[:acadamic_year])#, :mg_subject_type_id=>params[:mg_subject_type_id])
  puts ""
  ...
  if time_table.present?
    check_time_table=MgTimeTableChangeEntry.where(:mg_time_table_entry_id=>time_table[0].id, :mg_batch_id=>params[:mg_batch_id], :mg_school_id=> mg_school_id, :mg_class_timing_id=>params[:mg_class_timing_id], :mg_timetable_id=>params[:acadamic_year], :is_deleted=>0, :date=>Date.strptime(params[:date],school.date_format), :is_permanent=>0)
      ...
      if check_time_table.present?
        ...
        timetable_save.mg_subject_type_id=params[:mg_subject_type_id]
...
def elective_room_list    
  @subjects = MgSubject.where(:mg_elective_group_id=>params[:elective_group_id],:is_deleted=>0,:mg_school_id=>session[:current_user_school_id])
  @weekday_id_list = params[:weekday_id_list]
  @mg_class_timings_id=params[:mg_class_timings_id]
  @elective_group_id=params[:elective_group_id]
  @batch_id = params[:batch_id]
...
def change_teacher_permanent_save
  ...
  school=MgSchool.find(mg_school_id)
      MgTimeTableEntry.transaction do
        time_table=MgTimeTableEntry.find_by(:is_deleted=>0, :mg_school_id=>mg_school_id , :mg_batch_id=>params[:mg_batch_id], :mg_class_timings_id=>params[:mg_class_timing_id], :mg_timetable_id=>params[:acadamic_year])#, :mg_subject_type_id=>params[:mg_subject_type_id])
        puts ""
        puts time_table.inspect
        puts ""
...
def change_teacher_edit_save
  ...
  time_table.mg_subject_type_id=params[:mg_subject_type_id]
...
def mandatory_subject_room
  # mg_subject_id=MgEmployeeSubject.where(:mg_employee_id=>params[:mg_employee_id],:is_deleted=>0,:mg_school_id=>session[:current_user_school_id]).pluck(:mg_subject_id)
  @subject=MgSubject.find_by(:id=>params[:mg_subject_id])
  # if(subject.is_lab==true)
  #   @rooms=MgInstituteRoom.where(:mg_employee_specialization_id=>subject.mg_employee_specialization_id,:mg_room_type=>"practical").pluck(:room_no,:id)
  # else
  #   @rooms=MgInstituteRoom.where(:mg_employee_specialization_id=>subject.mg_employee_specialization_id,:mg_room_type=>"theory").pluck(:room_no,:id)
  # end
  render :layout=>false
end
