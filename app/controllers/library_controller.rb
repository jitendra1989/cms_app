...
def asst_library_incharge_edit
  ...
  def library_select_employees
    ...
    employee_category=MgEmployeeCategory.find_by(:category_name=>"Non Teaching Staff")
    if params[:data]=="asst"
      ...
      if employee_inc_data.length>0
       @deptid=params[:dept_data]
      @employeeList=MgEmployee.where("mg_employee_department_id=? AND mg_employee_category_id=? AND is_deleted=? AND mg_school_id=? AND id NOT IN (?)",@deptid,employee_category.id,0,session[:current_user_school_id],employee_inc_data)
   ...
   # debug
@employeeList=MgEmployee.where(:mg_employee_department_id=>@deptid,:mg_employee_category_id=>employee_category.id,:is_deleted=>0,:mg_school_id=>session[:current_user_school_id])
...
#logger.oaisdhfi
    ...
    if dept_data.present?
      ...
      @employeeList=MgEmployee.where("mg_employee_department_id=? AND mg_employee_category_id=? AND is_deleted=? AND mg_school_id=? AND id NOT IN (?)",@deptid,employee_category.id,0,session[:current_user_school_id],employee_asst_inc_data)
...
def library_incharge_data_update
 ...
 if library_employee_data.length>0
  ...
  def index
    ...
    if user.user_type=="admin" || incharge>0 || user.user_type=="principal"
  ...
  def delete
    @library=MgResourceCategory.find_by(:id=>params[:id])
    @library.update(:is_deleted=>1 ,:updated_by=>session[:user_id])
    redirect_to :back
  end
   ...
   if params[:library][:is_non_issuable].to_i==1
    #logger.infoasD
     library.update(:name=>params[:library][:name],:description=>params[:library][:description],:mg_resource_category_id=>params[:library][:mg_resource_category_id],:max_issuable_count=>params[:library][:max_issuable_count],:max_borrow_day=>params[:library][:max_borrow_day],:renewal_period=>params[:library][:renewal_period],:max_renewals_allowed=>params[:library][:max_renewals_allowed],
      :prefix=>params[:library][:prefix],:is_non_issuable=>params[:library][:is_non_issuable],:updated_by=>session[:user_id])
   else
...
def purchase_delete
  @library_purchase_details=MgResourcePurchase.find(params[:id])
  @library_purchase_details.update(:is_deleted=>1)
  @book_purchase_details=MgResourceInformation.where(:mg_resource_purchase_id=>params[:id],:is_deleted=>0,:mg_school_id=>session[:current_user_school_id])  
  @book_purchase_details.each do |purchase_detail|
    purchase_detail.update(:is_deleted=>1)
  end
  redirect_to :back
end
...
def kiosk_create
  ...
  if array.include?(false)
    flash[:error]="Please check duplicate user name"
  else
...
def kiosk_update
  ...
  array.push(kiosk.update(params_user_update))
  if array.include?(false)
    flash[:error]="Please check duplicate user name"
  else
    flash[:notice]="Successfully created"
...
# else
    @pass = params[:library]
    @passw = @pass[:password]  #old pass
    @rpass = params[:library]  #new pass
