class EventsController < ApplicationController
	...
	require 'json'
		...
		def  send_mail_to_member(event_committee, event_msg)
					event_committee.each do |user_find|
						if !user_find.mg_employee_id.present?
							...
							if user_id.present?
								send_email_create(user_id.mg_user_id, event_msg)

							end
						else
							user_id=MgEmployee.find_by(:id=>user_find.mg_employee_id)
							if user_id.present?
								@notice=send_email_create(user_id.mg_user_id, event_msg)
							end
						end
					end
		...
		def send_email_create(user, event_msg)
		 begin
 ...
 def server_description(code_s)
    ...
    case code_s.to_s
		...
		def delete
			...
			@events=MgEvent.find(params[:id])
			@events.is_deleted =1
			@events.updated_by =session[:user_id]
			@events.save
			all_users=[]
  ...
  private
  def events_params
    params.require(:events).permit(:is_fee_applicable,:amount,:mg_event_committee_id, :end_date,:title, :mg_event_type_id, :event_privacy, :event_description,  :start_time, :end_time, :all_day, :editable, :is_deleted, :event_date, :mg_school_id,  :created_by, :updated_by)
  end
