class DashboardsController < ApplicationController
  ...
  layout "mindcom"
  #com
def employee_question_paper_new


 render :layout=>false

end
def employee_question_paper_delete
  data=params[:id]
demo=data.split("-")

question_paper_data=MgEmployeeQuestionPaper.find_by(:id=>params[:id])
question_paper_data.is_deleted=1
question_paper_data.save
document_data=MgDocumentManagement.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_employee_question_paper_id=>demo[0])
document_data.each do |data|
data.delete
end
if demo[1]=="demo"
redirect_to :action=>"employee_question_paper_hod"

else
redirect_to :action=>"employee_question_paper"

end

end

def send_question_paper_to_hod
data=params[:id]
demo=data.split("-")
...
def employee_question_paper_update
question_paper_data=MgEmployeeQuestionPaper.find_by(:id=>params[:id])
 question_paper_data.mg_time_table_id=params[:question_paper_data][:mg_time_table_id]
 question_paper_data.mg_wing_id=params[:mg_wing_id]
 question_paper_data.mg_course_id=params[:mg_course_id]
 question_paper_data.mg_subject_id=params[:mg_subject_id]
 question_paper_data.mg_user_id=session[:user_id]
if question_paper_data.save
  if params[:question_paper_data][:file].present?
document_data=MgDocumentManagement.new(employee_document_managements_params)
document_data.mg_course_id=question_paper_data.mg_course_id
document_data.mg_subject_id=question_paper_data.mg_subject_id
document_data.is_deleted=0
document_data.document_type="EmployeeQuestionPaper"
 document_data.mg_school_id=session[:current_user_school_id]
 document_data.created_by=session[:user_id]
 document_data.updated_by=session[:user_id]
 document_data.mg_user_id=session[:user_id]
 document_data.mg_employee_question_paper_id=question_paper_data.id
 data=MgEmployeeHodLogin.find_by(:mg_user_id=>session[:user_id],:is_deleted=>0,:mg_school_id=>session[:current_user_school_id])
  if data.present?
 empl_hod_count=MgEmployee.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:id=>data.mg_employee_id,:is_hod=>true).count
                        if empl_hod_count>0
                          document_data.is_hod=true
                        end
                      end
 document_data.save
else
...
@question_paper_data=MgEmployeeQuestionPaper.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_user_id=>session[:user_id])
end
def employee_question_paper_hod

@question_paper_data=MgEmployeeQuestionPaper.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_user_id=>session[:user_id])
end
def employee_question_paper_show
@question_paper_data=MgEmployeeQuestionPaper.find_by(:id=>params[:id])
 render :layout=>false

  end
  def employee_question_paper_edit
@question_paper_data=MgEmployeeQuestionPaper.find_by(:id=>params[:id])
 render :layout=>false

   end 

def employee_question_paper_create
puts params.inspect
puts session[:user_id]
#data=MHodLogin.find_by(:mg_user_id=>session[:user_id],:is_deleted=>0,:mg_school_id=>session[:current_user_school_id])
#puts data .inspect
#puts aklshkjdhjkas
question_paper_data=MgEmployeeQuestionPaper.new
 question_paper_data.mg_time_table_id=params[:question_paper_data][:mg_time_table_id]
 question_paper_data.mg_wing_id=params[:mg_wing_id]
 question_paper_data.mg_course_id=params[:mg_course_id]
 question_paper_data.mg_subject_id=params[:mg_subject_id]
 question_paper_data.is_deleted=0
 question_paper_data.mg_school_id=session[:current_user_school_id]
 question_paper_data.created_by=session[:user_id]
 question_paper_data.updated_by=session[:user_id]
 question_paper_data.mg_user_id=session[:user_id]
if question_paper_data.save
document_data=MgDocumentManagement.new(employee_document_managements_params)
document_data.mg_course_id=question_paper_data.mg_course_id
...
document_data.is_deleted=0
document_data.document_type="EmployeeQuestionPaper"
data=MgExaminationHodLogin.find_by(:mg_user_id=>session[:user_id],:is_deleted=>0,:mg_school_id=>session[:current_user_school_id])
  if data.present?
  empl_hod_count=MgEmployee.find_by(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:id=>data.mg_employee_id,:is_hod=>true)
  if empl_hod_count.present?
document_data.is_hod=true
  end
end
 ...
 document_data.mg_user_id=session[:user_id]
 document_data.mg_employee_question_paper_id=question_paper_data.id
   # empl_hod_count=MgEmployee.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_user_id=>session[:user_id],:is_hod=>true).count
   #                      if empl_hod_count>0
   #                        document_data.is_hod=true
   #                      end
 document_data.save
...
def exam_management_select_subject
  if params[:impos_ids].present?
question_paper_data=MgEmployeeQuestionPaper.find_by(:id=>params[:impos_ids])
@subject_data123=question_paper_data.mg_subject_id
else
@subject_data123=0
 end
 @hod_data=MgExaminationHodLogin.find_by(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_user_id=>session[:user_id])
if @hod_data.present?
 employee_data=MgEmployee.find_by(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:id=>@hod_data.mg_employee_id)

 #if @employee_data.is_hod==true

 @subject=MgSubject.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_course_id=>params[:id],:mg_employee_specialization_id=>employee_data.hod_specialization_id).pluck(:subject_name,:id).uniq
 render :layout=>false

else
 @employee_data=MgEmployee.find_by(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_user_id=>session[:user_id])

  employee_subject=MgEmployeeSubject.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_employee_id=>@employee_data.id).pluck(:mg_subject_id).uniq
   @subject_data=MgSubject.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_course_id=>params[:id],:id=>employee_subject).pluck(:subject_name,:id)
 render :layout=>false
end

  end
def exam_management_select_class  


#   empl_hod_count=MgEmployee.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_user_id=>session[:user_id],:is_hod=>true).count
#   if empl_hod_count>0

#   empl_hod=MgEmployee.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_user_id=>session[:user_id],:is_hod=>true)
# employee_subject=MgEmployeeSubject.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_employee_id=>empl_hod.id).pluck(:mg_subject_id).uniq

#  @subject_data=MgSubject.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:id=>employee_subject).pluck(:mg_course_id).uniq

#  @course_data=MgCourse.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_wing_id=>params[:id],:id=>@subject_data).pluck(:course_name,:id)
#  render :layout=>false
#   else
  puts params[:impo_id].inspect
  #puts lLSDJLA
   if params[:impo_id].present?
question_paper_data=MgEmployeeQuestionPaper.find_by(:id=>params[:impo_id])
puts question_paper_data.inspect
@course_data123=question_paper_data.mg_course_id
else
@course_data123=0
end
    @hod_data=MgExaminationHodLogin.find_by(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_user_id=>session[:user_id])
if @hod_data.present?
   ...
   @empl_data=MgEmployee.find_by(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:id=>@hod_data.mg_employee_id)
#if @empl_data.present?
#if @empl_data.is_hod==true

 @subject=MgSubject.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_employee_specialization_id=>@empl_data.hod_specialization_id).pluck(:mg_course_id).uniq
 @course=MgCourse.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_wing_id=>params[:id],:id=>@subject).pluck(:course_name,:id)
 render :layout=>false
   ...
   empl_data=MgEmployee.find_by(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_user_id=>session[:user_id])


 employee_subject=MgEmployeeSubject.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_employee_id=>empl_data.id).pluck(:mg_subject_id).uniq
...
#end
def hod_question_paper


end
def hod_question_paper_list
...
def controller_question_paper


end
def controller_question_paper_list

#puts jhdga
 render :layout=>false

end
def exam_management_select_section
 

@batch_data=MgBatch.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:mg_course_id=>params[:id]).pluck(:name,:id)
 render :layout=>false
...
#leave status
  ...
  @employee=employee
      ...
      @mg_employee_leave_types=MgEmployeeLeaveType.where("is_deleted  = 0 AND mg_school_id = #{session[:current_user_school_id]} AND minimum_month_experience <= ? AND gender IN (?) AND mg_employee_type_id =? AND marital_status IN (?)",emp_experience, emp_gender, emp_type_id,emp_marital_status)
...
#     render :layout => false
 ...
 @academic_years  =   MgTimeTable.where("start_date < \"#{Date.today}\" && \"#{Date.today}\" < end_date").where(:is_deleted=>0,:mg_school_id=>@mg_school_id)
    ...
    timetables=ShowTimetable.new
    time_table_arr=timetables.student_show_time_table(@student.mg_batch_id, @mg_school_id, @academic_years[0].try(:id), session[:user_type], session[:user_id])
    if time_table_arr.present?
      @timetable=time_table_arr[0]
      @big_weekday=time_table_arr[1]
    end
...
id=session[:user_id]
  ...
  def student_timetable
    ...
    timetables=ShowTimetable.new
    time_table_arr=timetables.student_show_time_table(@batch_id, mg_school_id, @academic_years[0].try(:id), session[:user_type], session[:user_id])
    if time_table_arr.present?
      @timetable=time_table_arr[0]
      @big_weekday=time_table_arr[1]
    end
  ...
  mg_school_id=session[:current_user_school_id]
   ...
   if request.xhr?
    if(params[:calenderType]=='Student')
            ...
            render :json=> student_array#, objArray
      elsif (params[:calenderType]=='Alumni')

            puts "session[:user_id] --> #{session[:user_id]}"

            # =MgAlumni.find_by(:is_deleted=>0, :mg_school_id=>mg_school_id, :mg_user_id=>session[:user_id]).try(:id)
            
            alumni=MgAlumniProgrammeAttended.where(:is_deleted=>0,:mg_school_id=>mg_school_id,:mg_alumni_id=>MgAlumni.find_by(:is_deleted=>0, :mg_school_id=>mg_school_id, :mg_user_id=>session[:user_id]).try(:id))
            
            
            event=MgInvitation.where(:alumni=>1, :is_deleted=>0, :mg_school_id=>mg_school_id, :time_table_year=>alumni.pluck(:time_table_year),:mg_wing_id=>alumni.pluck(:mg_wing_id)).pluck(:mg_event_id)
            sql="select a.title, CONCAT(a.event_date,'T', DATE_FORMAT(a.start_time,'%H:%m:%s')) 'start',CONCAT(a.end_date,'T', DATE_FORMAT(end_time,'%H:%m:%s')) 'end', b.event_color 'color' , a.id from mg_events a , mg_event_types b where a.mg_event_type_id=b.id and a.is_deleted=0 and b.is_deleted=0 and  a.mg_school_id=#{mg_school_id} and b.mg_school_id=#{mg_school_id} and a.id in( #{event.present? ? event.compact.map(&:inspect).join(', ') : 0 })"
            @studentcalender=MgEvent.find_by_sql sql
            puts  @studentcalender.as_json.inspect
            render :json=> @studentcalender.as_json
...
def employee_document_managements_params
      params.require(:question_paper_data).permit(:file)
    end
