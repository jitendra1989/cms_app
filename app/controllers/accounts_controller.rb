    ...
    # =============================================================================================
      begin
        ...
        @central_incharge.each do |officer|
          user=MgUser.find_by(:id=>officer.mg_user_id)
          sender=MgUser.find_by(:id=>session[:user_id])
          subject="Account transfer request from "+sender.try(:user_name)
          if params[:account][:purpose].present?
              description=params[:account][:purpose]
          else
              description=""
          end
          notification=MgNotification.send_notification(session[:user_id],user.id,subject,description,session[:current_user_school_id],0,session[:user_id],session[:user_id])
          notification.save

          email=MgNotification.send_email(sender.email,user.email,subject,description,session[:current_user_school_id])
          email.save
