class ApplicationController < ActionController::Base
  ...
  def login_required
    puts "|---> login_required login_requiredlogin_requiredlogin_requiredlogin_required"
    if session[:user_id].present? && ((MgUser.find_by(:id=>session[:user_id], :is_deleted=>0).try(:mg_school_id).to_s==session[:current_user_school_id].to_s) || (MgMultiSchoolAccess.where(:is_deleted=>0, :mg_user_id=>session[:user_id]).pluck(:mg_school_id).include?(session[:current_user_school_id].to_i)))
