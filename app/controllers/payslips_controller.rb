...
def payment
  @payment=MgEmployeePayslipDetail.find_by(:id=>params[:id])
  @payment_count=MgAccountTransaction.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:for_module=>"payslip",:mg_particular_id=>params[:id]).count
  @central_payment_count=MgCentralAccountTransaction.where(:is_deleted=>0,:mg_school_id=>session[:current_user_school_id],:for_module=>"payslip",:mg_particular_id=>params[:id]).count
  render :layout=>false
end

def payment_updates
  @payment=MgEmployeePayslipDetail.find_by(:id=>params[:id])
  amount=@payment.try(:total_net_amount)
  to_account_id=""
  amount_transfer_type="debited"
  for_module="payslip"
  particular_id=params[:id]
  transaction_type="payable"
  if params[:mg_account_id]=="central"
    ...
    from_account_id=params[:mg_account_id]
    account_transaction=MgAccountTransaction.add_transaction(from_account_id,to_account_id,amount.abs,for_module,particular_id,transaction_type,amount_transfer_type,session[:current_user_school_id],session[:user_id],session[:user_id])
    account_transaction.save
  end
  redirect_to :action=>'index'
