class MgStudentAdmissionsController < ApplicationController
  ...
  def create
    ...
    MgStudentAdmission.transaction do
      ...
      if @admission.save
        @admission.update(:created_by=>session[:user_id])
  ...
  def destroy
    ...
    @admission.update(:updated_by=>session[:user_id])
  ...
  def params_student_data
    params.require(:mg_student_admission).permit(:updated_by,:mg_school_id,:is_deleted,:mg_student_category_id,
