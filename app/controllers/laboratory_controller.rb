class LaboratoryController < ApplicationController
  ...
  def delete
    ...
    @lab.update(:is_deleted=>1,:updated_by=>session[:user_id])
    redirect_to :back
  end
  ...
  def update
    @lab=MgLab.find(params[:id])
    @lab.update(:room_no=>params[:laboratry][:room_no],:lab_name=>params[:laboratry][:lab_name],:mg_laboratory_subject_id=>params[:laboratry][:mg_laboratory_subject_id],:updated_by=>session[:user_id])
    redirect_to :back
  end
  def inventory
  ...
  def inventory_update
    ...
    @lab.update(:mg_lab_id=>params[:laboratry][:mg_lab_id],:category_name=>params[:laboratry][:category_name],:mg_laboratory_item_id=>params[:laboratry][:mg_laboratory_item_id],:prefix=>params[:laboratry][:prefix],:updated_by=>session[:user_id])
    redirect_to :action => "inventory"
  end
...
def purchase
  ...
  def purchase_create
   # begin
        ...
        item_purchase.updated_by=session[:user_id]
  ...
  def purchase_update
    ...
    @laboratory_purchase_details.update(:updated_by=>session[:user_id])
    @timeformat = MgSchool.find(session[:current_user_school_id])
 ...
 if laboratory_data.length>0
    for j in 0...laboratory_data.length 
     ...
     if data.present?
    data.update(:is_deleted=>1,:updated_by=>session[:user_id])
  end
  end
   end
   for j in 0...@params.size
...
item_information_detail.updated_by=session[:user_id]
...
if valid_upto_arr[j].present?
    ...
    laboratry_data[0].update(:mg_category_id=>mg_category_id_arr[j],:mg_purchase_id=>params[:id],:item_name=>item_name_arr[j],:valid_upto=>valid_upto_arr[j],:total=>total_arr[j],:cost=>cost_arr[j],:is_deleted=>0,:mg_school_id=>school_id,:unit=>unit_arr[j],:updated_by=>session[:user_id])
    valid_date = Date.strptime(valid_upto_arr[j],@timeformat.date_format)
  ...
  def purchase_delete
    ...
    item_pur.update(:is_deleted=>1,:updated_by=>session[:user_id])
    ...
    item_info.each do |item|
     item.update(:is_deleted=>1,:updated_by=>session[:user_id])
    end
    redirect_to :action =>"purchase"
...
def subject_delete
    @laboratory=MgLaboratorySubject.find_by(:id=>params[:id])
    @laboratory.update(:is_deleted=>1,:updated_by=>session[:user_id])
    redirect_to :back
end
