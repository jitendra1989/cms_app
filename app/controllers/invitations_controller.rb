class InvitationsController < ApplicationController
	...
	def new
		...
		@alumin_list=MgInvitation.where(:is_deleted=>0,:mg_school_id=>mg_school_id, :mg_event_id=>@mg_event_id,:alumni=>1)
		@alumni_programme_list=MgAlumniProgrammeAttended.where(:is_deleted=>0, :mg_school_id=>session[:current_user_school_id]).order(:time_table_year,:mg_wing_id).uniq(:time_table_year)
		render :layout => false
	end
	...
	def create
		...
		alumni_arr=[]#params[:alumni_mg_wing_id_time_table_year]

		if params[:alumni_mg_wing_id_time_table_year].present? && params[:invitation][:alumni]=='1'
			params[:alumni_mg_wing_id_time_table_year].each do |v|
				alumni_arr +=MgAlumniProgrammeAttended.where(:mg_school_id=>mg_school_id, :is_deleted=>0,:mg_wing_id=>v.to_s.split("-")[0],:time_table_year=>v.to_s.split("-")[1]).pluck(:mg_alumni_id)
				# alumni_arr
				# puts "alumni_arr===>#{alumni_arr.inspect}"
			end
		end
		
		alumni_arr_user=MgAlumni.where(:id=>alumni_arr, :is_deleted=>0,:mg_school_id=>mg_school_id)#.pluck(:mg_user_id)
		# puts "alumni_arr===>#{alumni_arr.inspect}"
		# puts "alumni_arr_user-->#{alumni_arr_user.inspect}"
		...
		# puts dhfashfyhasgh
		   ...
		   begin
		     MgInvitation.transaction do
				...
				# @albums=MgAlbum.new(albums_params)
					...
					if var.present?
				  		...
				  		if params[:mg_employee_department_id].present?
			  				...
			  				for i in 0...params[:mg_employee_department_id].size
			  					...
			  					if present_date.present?
			  						present_date.update(:is_deleted=>0, :employee=>1, :updated_by=>created_by)
			  					else
				  	...
				  	if params[:invitation][:teacher]=='1'
				  		if params[:tech_mg_employee_department_id].present?
			  				...
			  				for i in 0...params[:tech_mg_employee_department_id].size
			  					...
			  					if present_date.present?
			  						present_date.update(:is_deleted=>0, :teacher=>1, :updated_by=>created_by)
			  					else
				  	...
				  	if params[:invitation][:alumni]=='1'
				  		if params[:alumni_mg_wing_id_time_table_year].present?
				  				puts params[:alumni_mg_wing_id_time_table_year].inspect
				  				for i in 0...params[:alumni_mg_wing_id_time_table_year].size
					  				present_date=MgInvitation.find_by(:mg_school_id=>mg_school_id, :mg_event_id=>params[:invitation][:mg_event_id], :mg_wing_id=>params[:alumni_mg_wing_id_time_table_year][i].to_s.split("-")[0],:time_table_year=> params[:alumni_mg_wing_id_time_table_year][i].to_s.split("-")[1],:alumni=>1)
				  					if present_date.present?
				  						present_date.update(:is_deleted=>0, :updated_by=>created_by)
				  					else
				  						event=MgInvitation.new(:mg_event_id=>params[:invitation][:mg_event_id],:is_deleted=>0, :mg_school_id=>mg_school_id, :mg_wing_id=>params[:alumni_mg_wing_id_time_table_year][i].to_s.split("-")[0],:time_table_year=> params[:alumni_mg_wing_id_time_table_year][i].to_s.split("-")[1], :alumni=>1, :created_by=>created_by, :updated_by=>created_by)
				  						event.save
				  					end
			  					end
				  		end
			...
			@notice=email_and_notification(alumni_arr_user,params[:guardian_mg_batch_id],params[:student_mg_batch_id],params[:mg_employee_department_id],params[:tech_mg_employee_department_id],params[:invitation][:mg_event_id])
	...
	def email_and_notification(alumni_arr_user,guardian_mg_batch_id,student_mg_batch_id, mg_employee_department_id,tech_mg_employee_department_id, mg_event_id)
		mg_school_id=session[:current_user_school_id]
		all_users=[]
		# puts "fogbjighipsghiphipjhiofh-----#{alumni_arr_user.inspect}"
		...
		if alumni_arr_user.present?
			alumni_arr_user.each do |s|
				@notice=send_email_alumni(s,s.mg_user_id,msg,event_suject)
			end
		end
	...
	def send_email_alumni(obj,user, event_msg, event_suject)
				begin
			...
			@message.description =event_msg
						...
						# all_user.each do |user|
					    	@email_to = MgEmail.where(:mg_user_id => user)
				            	...
				            	if obj.try(:email_id).present?
						            @message.to_user_id =obj.try(:email_id)
