        ...
        $.ajax({
            ...
            success: function(html){
                  $('#schoolDialogID').dialog({
                    ...
                    open: function () {
                        $(this).html(html);
                          /*$(this).window.parent.scroll(function(){
    alert("popup");
    if ($(this).scrollTop() > 100) {
      $('.mg-scrollToTop-popup').fadeIn();
    } else {
      $('.mg-scrollToTop-popup').fadeOut();
    }
  });*/
                    }
...
$(document).on("click", ".delete-school-room-btn", function(e){
     e.preventDefault();
                var myID =$(this).attr('id');
            var Id=myID.split("-");
                var retVal = confirm("Are you sure to delete?");
                if(retVal){
                  //  alert(retVal);
                     var urlLink = "/schools/room_delete/"+Id[0];
                      $.ajax({
                        url: urlLink ,
                        cache: false,
                        success: function(html){
