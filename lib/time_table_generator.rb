class TimeTableGenerator
  ...
  Subject  = Struct.new(:id,:sub_code,:section_count,:emp_count,:class_id,:max_classes,:is_core_subject,:is_lab,:no_of_classes,:room_id,:available_sections,:allotted_sections,:mg_subject_type_id,:mg_elective_group_id)
  ...
  Entry = Struct.new(:mg_weekday_id,:mg_class_timings_id,:mg_batch_id,:mg_subject_id,:mg_employee_id,:mg_school_id,:mg_room_id,:mg_subject_type_id,:mg_elective_group_id)
  ...
  def create_time_table(school_id)
      ...
      #checking class teacher subjects
       ...
       all_batches.each do |batch|
         ...
         if(class_teacher_id!=nil)
           class_subjects = MgSubject.where(:is_deleted=>0,:mg_school_id=>school_id,:mg_course_id=>batch.mg_course_id,:mg_subject_type_id => 1).pluck(:id)

           puts "12345qwertt"
           puts class_subjects.inspect
           #shr345 start
           elective_groups =  MgElectiveGroup.where(:is_deleted=>0,:mg_school_id=>school_id).pluck(:id)

           elective_groups.each do |curr_group|
             group_subjects = MgSubject.where(:is_deleted=>0,:mg_school_id=>school_id,:mg_course_id=>batch.mg_course_id,:mg_elective_group_id => curr_group).pluck(:id)

             if group_subjects.size>0
               class_subjects << group_subjects.first
             end
      ...
      course_list.each do |course|
        ...
        total_lectures = 0
        subjects = MgSubject.where(:is_deleted=>0,:mg_school_id=>school_id,:mg_course_id=>course.id,:mg_subject_type_id => 1)
        ...
        elective_groups.each do |curr_group|
          group_subjects = MgSubject.where(:is_deleted=>0,:mg_school_id=>school_id,:mg_course_id=>course.id,:mg_elective_group_id => curr_group)

          if group_subjects.size>0
            subjects << group_subjects.first
          end
        ...
        subjects.each do |sub|
          ...
          if(sub.is_lab)
            ...
            emp_subject_list.each do |es|
              ...
              active_subject_ids = MgSubject.where(:is_deleted=>0,:mg_school_id=>school_id,:mg_subject_type_id => 1).pluck(:id)
...
#shr123start
          curr_sub =  MgSubject.find(curr_entry['mg_subject_id'])
          specialization = curr_sub.mg_employee_specialization_id
          room_list = MgInstituteRoom.where(:mg_school_id => school_id,:mg_employee_specialization_id => specialization,:is_deleted => 0).pluck(:id);


          time_table_entry.mg_subject_type_id = curr_sub.mg_subject_type_id
          time_table_entry.mg_elective_group_id = curr_sub.mg_elective_group_id


          if curr_sub.is_lab

            time_table_entry.mg_institute_room_id = curr_sub.mg_institute_room_id
            time_table_entry.save
          elsif curr_sub.mg_subject_type_id == 1

            room_list.each do |curr_room|

              not_available = MgTimeTableEntry.where(:is_deleted=>0, :mg_school_id=>school_id, :mg_institute_room_id=>curr_room, :mg_weekday_id=>week_day['id'], :mg_class_timings_id=>timing.id).present?

              if !not_available
                time_table_entry.mg_institute_room_id = curr_room
                break;
              end
          ...
          if curr_sub.mg_subject_type_id == 2

            elective_subjects = MgSubject.where(:is_deleted=>0, :mg_school_id=>school_id, :mg_elective_group_id => curr_sub.mg_elective_group_id,:mg_course_id => curr_sub.mg_course_id)

            elective_subjects.each do |curr_elective|


              elective_entry= MgTimeTableEntry.new

              elective_entry.mg_weekday_id=week_day['id']
              elective_entry.mg_batch_id=curr_entry['mg_batch_id']
              elective_entry.mg_timetable_id=time_table_id
              elective_entry.mg_school_id=curr_entry['mg_school_id']
              elective_entry.is_deleted=0
              elective_entry.mg_class_timings_id=timing.id
              elective_entry.mg_subject_type_id = curr_sub.mg_subject_type_id
              elective_entry.mg_elective_group_id = curr_sub.mg_elective_group_id

              elective_entry.mg_subject_id=curr_elective.id

              elective_employees = MgEmployeeSubject.where(:mg_subject_id => curr_elective.id,:is_deleted=>0, :mg_school_id=>school_id).pluck(:mg_employee_id)


              elective_employees.each do |curr_emp|


                not_available = MgTimeTableEntry.where(:is_deleted=>0, :mg_school_id=>school_id, :mg_employee_id=>curr_emp, :mg_weekday_id=>week_day['id'], :mg_class_timings_id=>timing.id).present?

                if !not_available
                  elective_entry.mg_employee_id = curr_emp
                  break;
                end
              ...
              not_available = MgTimeTableEntry.where(:is_deleted=>0, :mg_school_id=>school_id, :mg_institute_room_id=>curr_room, :mg_weekday_id=>week_day['id'], :mg_class_timings_id=>timing.id).present?
                  ...
                  if !not_available
                    elective_entry.mg_institute_room_id = curr_room
                    break;
                  end
...
#shr123end
  ...
  def create_time_table_specialization(school_id,spec_id)
    puts "Creating Timetable Spec"
    ...
    spec_employees  = MgParticularSpecialization.where(:is_deleted => 0,:mg_school_id => school_id,:mg_employee_specialization_id => spec_id).pluck(:mg_employee_id)
    ...
    all_batches = MgBatch.where(:is_deleted => 0,:mg_school_id => school_id,:mg_employee_id => spec_employees)
    ...
    all_batches.each do |batch|
      ...
      if(class_teacher_id!=nil)
        class_subjects = MgSubject.where(:is_deleted=>0,:mg_school_id=>school_id,:mg_course_id=>batch.mg_course_id,:mg_employee_specialization_id => spec_id,:mg_subject_type_id => 1).pluck(:id)
    ...
    @all_class_teacher_id_list = MgBatch.where(:is_deleted => 0,:mg_school_id => school_id,:mg_employee_id => spec_employees).pluck(:mg_employee_id)
    ...
    course_list.each do |course|
      ...
      batch_list=MgBatch.where(:is_deleted=>0,:mg_school_id=>school_id,:mg_course_id=>course.id,:mg_employee_id => spec_employees)

      total_lectures = 0
      subjects = MgSubject.where(:is_deleted=>0,:mg_school_id=>school_id,:mg_course_id=>course.id,:mg_employee_specialization_id => spec_id,:mg_subject_type_id => 1)
      ...
      subjects.each do |sub|
        ...
        emp_subject_list=MgEmployeeSubject.where(:is_deleted=>0,:mg_school_id=>school_id,:mg_subject_id=>sub.id)
        #emp_subject_list=MgEmployeeSubject.where(:is_deleted=>0,:mg_school_id=>school_id,:mg_subject_id=>sub.id,:mg_employee_id => spec_employees)
        ...
        emp_subject_list.each do |es|
          ...
          #--------initiating Employee Stats
          active_subject_ids = MgSubject.where(:is_deleted=>0,:mg_school_id=>school_id,:mg_employee_specialization_id => spec_id,:mg_subject_type_id => 1).pluck(:id)
          curr_emp_subjects = MgEmployeeSubject.where(:mg_employee_id=>es.mg_employee_id,:mg_subject_id=>active_subject_ids)
          course_id_list = MgSubject.where(:is_deleted=>0,:mg_school_id=>school_id,:id=>curr_emp_subjects.pluck(:mg_subject_id)).pluck(:mg_course_id).uniq
          section_count = MgBatch.where(:is_deleted=>0,:mg_school_id=>school_id,:mg_course_id=>course_id_list,:mg_employee_id => spec_employees).count
          emp = MgEmployee.find(es.mg_employee_id)
